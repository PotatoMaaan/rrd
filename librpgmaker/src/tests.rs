use crate::{rpg_file::RpgFile, Decrypted, Encrypted};
...
/// The decryption key for the above image
const KEY: &[u8] = &[
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
];

/// The hash of the image once it's decrypted with the above key
...
fn build_file() -> RpgFile<Encrypted> {
    unsafe {
        RpgFile::from_raw_parts_encrypted(
            IMG_ENC.to_vec(),
            crate::rpg_file::FileType::Image,
...
fn test_encrypt() {
    let file = build_file();
    let file = file.decrypt(KEY).unwrap();
    let file = file.encrypt(KEY).unwrap();

    assert_eq!(file.data, IMG_ENC);
}

#[test]
fn test_encrypt_fail() {
    let file = build_file();
    let file = file.decrypt(KEY).unwrap();
    let file = file.encrypt(&[0, 1, 2, 3, 4]).unwrap();

    assert_ne!(file.data, IMG_ENC);
}

#[test]
fn test_decrypt_fail() {
    let file = build_file();
    let file = file.decrypt(&[0, 0, 1, 1, 2, 2, 3, 3]).unwrap();
    let hash = sha256(&file.data);
